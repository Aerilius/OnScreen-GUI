...
# TODO: allow non-linear scale, option to show ruler
  def initialize(label="", minmax=[0,50,100], hash={}, &block)
    hash = hash.dup
    label_size = [ text_width(label), text_height(label) ]
    hash[:width] ||= (hash[:orientation]!=:vertical)? 200 : [label_size[0], 30].max
    hash[:height] ||= (hash[:orientation]!=:vertical)? [label_size[1], 30].max : 200
    super(hash)
    @data = {
      :label => label,
      :min => minmax.min,
  ...
  def trigger(type, data)
    if type == :mousedown || type == :move || type == :mouseup
      o = @style[:orientation]==:horizontal
      s = (self.size)[o ? 0 : 1]
      ...
      when :mousedown then
        ...
        @window.dragging = self if (-@data[:gripper_size][0]/2..@data[:gripper_size][0]/2) === data[:pos][o ? 0 : 1] - l - g
      when :move then
  ...
  def draw(view, pos, size)
    ...
    pos += o ? [@data[:label_size][0], 0] : [0, @data[:label_size][1]] if o && @style[:align]==:left || !o && @style[:valign]!=:bottom
    # label min
    ...
    if o
      ...
      slider_length -= @data[:label_size][0] if @style[:align] != :center
    else
      slider_length = size[1] - l12 - @data[:label_size][1]
    end
    slider_start_length = @value/(@data[:max]-@data[:min]).to_f * slider_length - 0.5*@data[:gripper_size][0]
    ...
    # gripper text
    _pos = pos + (o ? [0, -@data[:gripper_size][1]/2-15] : [-@data[:gripper_size][0]/2-28, 0])
    draw_text(view, _pos, sprintf("%1.3g", @value), style)
    # gripper
    _pos = pos + (o ? [0, -@data[:gripper_size][1]/2] : [-@data[:gripper_size][0]/2, 0])
    g1 = @data[:gripper_size][0]
    g2 = @data[:gripper_size][1]
    gripper_polygon = o ? [ [0,0], [g1,0], [g1,g2-g1/2.0], [g1/2.0, g2], [0, g2-g1/2.0], [0,0] ] : [ [0,0], [g2-g1/2.0, 0], [g2, g1/2.0], [g2-g1/2.0, g1], [0,g1], [0,0] ]
    draw_polygon(view, _pos, gripper_polygon, style)
    pos += o ? [g1, 0] : [0, g1]
    # slider end
    _pos = pos + (o ? [0, -2] : [-2, 0])
    slider_end_length = slider_length - @data[:gripper_size][0] - slider_start_length
